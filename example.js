...
const VendorDashboard = ({ navigation }) => {
  ...
  const checkConnection = async () => {
    try {
      ...
      const response = await fetch("http://192.168.254.111:3000/status");
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("http://192.168.254.111:3000/products");
      const data = await response.json();
      if (data.success) {
  ...
  const handleSubmitItem = async () => {
    ...
    try {
      ...
      // Append the image with proper file information
      ...
      const response = await fetch("http://192.168.254.111:3001/products", {
        method: "POST",
        body: formData,
        headers: {
